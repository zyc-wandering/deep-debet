...
def test_start_request_defaults():
    ...
    assert req.debater_count == 4
    assert req.max_turns == 24
    assert req.enable_debater_search is True
...
def test_invalid_debater_count_raises():
    with pytest.raises(Exception):
        DebateStartRequest(topic="Will AI replace PMs?", debater_count=3)
